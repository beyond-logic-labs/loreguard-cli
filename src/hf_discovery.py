...
def fetch_hf_repo_files(repo_id: str) -> list[GGUFFile]:
    ...
    try:
        ...
        print(f"[HF Discovery] Failed to fetch {repo_id}: {e}")
        return []
    ...
    for file_info in siblings:
        ...
        if filename.endswith(".gguf"):
            ...
            gguf_files.append(GGUFFile(filename=filename, size_bytes=size, url=url))
...
def fetch_org_repos(org: str = HF_ORG) -> list[str]:
    ...
    try:
        ...
        print(f"[HF Discovery] Failed to fetch org repos: {e}")
        return []

    # Filter for loreguard-* repos with gguf in name
    repos = []
    for repo in data:
        repo_id = repo.get("id", "")
        if repo_id.startswith(f"{org}/loreguard-") and "gguf" in repo_id.lower():
...
def discover_models(org: str = HF_ORG, use_cache: bool = True) -> list[ModelInfo]:
    ...
    print(f"[HF Discovery] Fetching models from {org}...")
    ...
    for repo_id in repos:
        ...
        for gguf_file in files:
            ...
            # Skip F16/BF16 from auto-discovery (too large for most users)
            if quant in ("F16", "BF16") and size_gb > 10:
                continue
    ...
    print(f"[HF Discovery] Found {len(models)} models")
    return models
